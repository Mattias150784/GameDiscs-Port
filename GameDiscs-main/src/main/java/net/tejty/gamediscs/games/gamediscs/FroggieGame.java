...
public class FroggieGame extends Game {
    ...
    public synchronized void render(PoseStack poseStack, int posX, int posY) {
        ...
        for (Sprite minecart : minecarts) {
            minecart.render(poseStack, posX, posY);
        }
        for (Sprite log : logs) {
            log.render(poseStack, posX, posY);
        }

        frog.render(poseStack, posX, posY);
        ...
        String scoreText = Component.literal(String.valueOf(score)).getString();
        font.drawShadow(poseStack, scoreText, posX + 2, posY + HEIGHT - font.lineHeight, 0xFFFFFF);
