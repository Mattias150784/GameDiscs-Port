...
public class BlocktrisGame extends Game {
    ...
    public synchronized void render(PoseStack poseStack, int posX, int posY) {
        ...
        grid.render(poseStack, posX + 45, posY - 15);
        piece.render(poseStack, posX + 45, posY - 15);

        for (int i = 0; i < nexts.size(); i++) {
            BlocktrisPiece next = nexts.get(i);
            next.setRotation(1);
            next.setPos(23, 4 + i * 3);
            next.renderCentered(poseStack, posX, posY);
        }

        if (hold != null) {
            hold.setRotation(1);
            hold.setPos(4, 6);
            hold.renderCentered(poseStack, posX, posY);
        }

        Font font = Minecraft.getInstance().font;
        Component holdComponent = Component.translatable("gui.gamingconsole.hold");
        String holdText = holdComponent.getString();
        int holdWidth = font.width(holdText);
        font.draw(poseStack, holdText, 22 + posX - holdWidth / 2, 16 + posY, 0x555555);

        Component nextComponent = Component.translatable("gui.gamingconsole.next");
        String nextText = nextComponent.getString();
        int nextWidth = font.width(nextText);
        font.draw(poseStack, nextText, 118 + posX - nextWidth / 2, 6 + posY, 0x555555);
